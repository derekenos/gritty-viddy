...
function convertPositionToIndex(col, row) {
  return 4 * (col + this.constants.width * (this.constants.height - 1 - row))
}
...
export const blurFilter = gpu.createKernel(function (data, params) {
  ...
  for (let lev = level; lev > 0; lev -= 1) {
    let nCol = col - lev
    if (nCol >= 0 && nCol < this.constants.width) {
      i = convertPositionToIndex(nCol, row)
      pixel[0] += data[i]
      pixel[1] += data[i + 1]
      pixel[2] += data[i + 2]
      pixel[3] += data[i + 3]
  ...
  .setFunctions([ getContext, setColor, convertPositionToIndex ])
  .setDynamicOutput(true)
  .setGraphical(true)
